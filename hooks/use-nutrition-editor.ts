...
export function useNutritionEditor({ planId, planStatus, trainerId }: UseNutritionEditorProps) {
  ...
  const handleAddMeal = async (form: AddMealForm): Promise<NutritionMealData | null> => {
    ...
    if (!form.mealType)    { toast.error("El tipo es obligatorio");   return null }
  ...
  const handleRemoveMeal = async (mealId: number): Promise<boolean> => {
    ...
    try {
      const response = await api.delete(`/api/nutrition-plans/meals/${mealId}?trainerId=${trainerId}`)
      if (!response.success) {
  ...
  const handleActivate = async () => {
    ...
    try {
      const response = await api.post(`/api/nutrition-plans/${planId}/activate?trainerId=${trainerId}`)
      if (!response.success) {
        toast.error("Error al activar plan", { description: response.error })
        return
      }
      toast.success("Plan activado", { description: "El miembro ya puede ver su plan nutricional." })
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await api.post(`/api/nutrition-plans/${planId}/cancel?trainerId=${trainerId}`)
      if (!response.success) {
