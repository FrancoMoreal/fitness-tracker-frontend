...
async function getWorkoutPlansByTrainer(trainerId: number): Promise<WorkoutPlanData[]> {
  try {
    const res = await fetch(`${API_URL}/api/workout-plans/trainers/${trainerId}`, {
...
export default async function TrainerRoutinesPage() {
  ...
  const plans = await getWorkoutPlansByTrainer(trainer.id)
  ...
  const planSummaries: PlanSummary[] = plans.map((p) => ({
    ...
    endDate:     p.endDate,
    totalItems:  p.totalDays,
  }))

  // Ordenar: borradores primero, luego activos, luego el resto
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Todos los planes de workout que creaste para tus miembros.
        </p>
      </div>

      <PlansList type="workout" plans={planSummaries} />
    </div>
  )
}