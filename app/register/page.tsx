...
export default function RegisterPage() {
  ...
  const validateForm = (): boolean => {
    console.log("[Register] Validando formulario", { userType, formData: { ...formData, password: "***", confirmPassword: "***" } })
    ...
    } else if (userType === "trainer" && !/^[a-zA-Z0-9_-]+$/.test(formData.username)) {
            ...
            <div className="mb-8 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary">
                  {userType === "member" ? <User className="h-6 w-6" /> : <Dumbbell className="h-6 w-6" />}
                </div>
                <div>
                  <h2 className="text-2xl font-bold">
                    Registro de {userType === "member" ? "Atleta" : "Entrenador"}
                  </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {userType === "trainer" && (
                <div className="space-y-4 rounded-lg border bg-muted/30 p-6">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="certificationsText">Certificaciones (una por línea o separadas por coma)</Label>
                    <InputGroup className={cn("h-auto min-h-[80px]", errors.certificationsText && "border-destructive ring-destructive/20")}>
